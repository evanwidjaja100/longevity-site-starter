	...
	private static function external_evidence( string $option, string $message ): array {
		...
		if ( is_string( $value ) ) {
			...
			if ( ! is_array( $decoded ) ) {
				...
				return array(
					'status'    => 'unknown_external',
					'message'   => 'Legacy evidence format; operator revalidation required.',
					'raw_value' => sanitize_text_field( substr( $value, 0, 100 ) ),
				);
			}
			$value = $decoded;
		}
		...
		if ( ! is_array( $value ) ) {
			return array(
				'status'  => 'unknown_external',
				'message' => 'Malformed evidence; operator revalidation required.',
			);
		}
		...
		if ( ! in_array( $result, array( 'ok', 'pass', 'fail', 'error' ), true ) ) {
			return array(
				'status'   => 'degraded',
				'message'  => 'Evidence result is missing or invalid.',
				'evidence' => $value,
			);
		}
		...
		if ( $performed_ts && $performed_ts > time() + 3600 ) {
			return array(
				'status'   => 'degraded',
				'message'  => 'Evidence performed_at is in the future.',
				'evidence' => $value,
			);
		}
		...
		if ( '' !== $environment && $environment !== $current_env ) {
			return array(
				'status'   => 'degraded',
				'message'  => sprintf( 'Evidence is for environment "%s" but current is "%s".', $environment, $current_env ),
				'evidence' => $value,
			);
		}
